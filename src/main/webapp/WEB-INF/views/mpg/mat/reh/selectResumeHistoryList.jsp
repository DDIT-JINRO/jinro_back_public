...
<div class="public-wrapper">
	...
	<div class="public-wrapper-main">
		<form method="GET" action="/mpg/mat/reh/selectResumeHistoryList.do" class="search-filter__form">
			<div class="search-filter__bar">
				<div class="search-filter__select-wrapper">
					<select name="status" class="search-filter__select">
						<option value="">전체</option>
						<option value="Y" <c:if test="${param.status eq 'Y'}">selected</c:if>>임시저장</option>
						<option value="N" <c:if test="${param.status eq 'N'}">selected</c:if>>작성완료</option>
					</select>
				...
				<div class="search-filter__input-wrapper">
					<input type="search" name="keyword" value="${param.keyword}" class="search-filter__input" placeholder="내가 작성한 이력서에서 검색">
				...
				<c:when test="${empty articlePage.content || articlePage.content == null}">
					<div class="empty-state">
						...
						<div class="empty-state__action">
							<a href="/cdp/rsm/rsm/resumeWriter.do" class="button button--primary button--large"> 이력서 작성하기 </a>
						</div>
					</div>
				</c:when>
				<c:otherwise>
