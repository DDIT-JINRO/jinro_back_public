		...
		<div class="public-wrapper-main">
				...
				<div class="search-filter__bar">
					<div class="search-filter__select-wrapper">
						<select name="status" class="search-filter__select">
							...
							<option value="counselName" ${param.status == 'counselName' ? 'selected' : ''}>상담사</option>
							<option value="content" ${param.status == 'content' ? 'selected' : ''}>후기 내용</option>
							<option value="writer" ${param.status == 'writer' ? 'selected' : ''}>작성자</option>
						</select>
					...
					<div class="search-filter__input-wrapper">
						<input class="search-filter__input" type="search" name="keyword" placeholder="상담 후기 게시판 내에서 검색" value="${param.keyword}">
									...
									<c:forEach var="method" items="${counselMethod}">
										<c:if test="${method.key ne 'G08004'}">
											...
											<label class="search-filter__option">
												<input type="checkbox" name="counselMethods" value="${method.key}" ${isChecked ? 'checked' : ''} />
							...
							<div class="search-filter__group">
								...
								<div class="search-filter__options">
									<c:forEach var="category" items="${counselCategory}">
										...
										<label class="search-filter__option">
											<input type="checkbox" name="counselCategorys" value="${category.key}" ${isChecked ? 'checked' : ''} />
			...
			<div class="content-list">
				<div class="accordion-list__header">
					<span class="accordion-list__col accordion-list__col--counselor">상담사</span>
					<div class="accordion-list__col accordion-list__col--meta-group">
						<span class="accordion-list__col accordion-list__col--method">상담방법</span>
						<span class="accordion-list__col accordion-list__col--category">상담분야</span>
						...
						<span class="accordion-list__col accordion-list__col--public">공개여부</span>
					</div>
				...
				<c:forEach var="content" items="${articlePage.content}">
					<div class="accordion-list__item">
						<div class="accordion-list__item-header">
							<div class="accordion-list__col accordion-list__col--counselor">${content.counselName}</div>
							<div class="accordion-list__col accordion-list__col--meta-group">
								<div class="accordion-list__col accordion-list__col--method">
									<span>${content.counselMethod}</span>
								</div>
								<div class="accordion-list__col accordion-list__col--category">
									<span>${content.counselCategory}</span>
								</div>
								<div class="accordion-list__col accordion-list__col--author">
								...
								<div class="accordion-list__col accordion-list__col--public">
							...
							<div class="accordion-list__content-body">
								...
								<c:if test="${pageContext.request.userPrincipal.principal == content.memId}">
									<div class="card-actions">
										<button type="button" class="card-actions__button card-actions__button--edit" data-mem-id="${content.memId}" data-cr-id="${content.crId}">
											수정
										</button>
										<button type="button" class="card-actions__button card-actions__button--delete" data-cr-id="${content.crId}">
											삭제
										</button>
									</div>
								</c:if>
				...
				<c:if test="${empty articlePage.content}">
					<p class="content-list__no-results">상담 후기가 없습니다.</p>
				</c:if>
			</div>
			
			<div class="page-actions">
				<button id="btnWrite" class="page-actions__button">상담 후기 공유하기</button>
			</div>
