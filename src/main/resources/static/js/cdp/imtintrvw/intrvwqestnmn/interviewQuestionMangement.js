...
document.addEventListener('DOMContentLoaded', function() {
	...
	const updateSelectedFiltersDisplay = () => {
		...
		if (checkedStatus) {
			...
			const tagHTML = `<span class="search-filter__tag" data-name="status" data-value="${checkedStatus.value}">상태 > ${labelText}<button type="button" class="search-filter__tag-remove">×</button></span>`;
			selectedFiltersContainer.insertAdjacentHTML('beforeend', tagHTML);
		}
		// 라디오 이벤트
		...
		if (selectedFiltersContainer) {
			selectedFiltersContainer.addEventListener('click', (e) => {
				if (e.target.classList.contains('search-filter__tag-remove')) {
					...
					//if (checkbox.dataset.preChecked == 'true') checkbox.dataset.preChecked = '';
					updateSelectedFiltersDisplay();
				}
			});
		}
