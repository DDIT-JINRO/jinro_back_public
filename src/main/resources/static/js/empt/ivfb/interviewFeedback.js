	...
	const updateSelectedFiltersDisplay = () => {
		...
		allCheckboxes.forEach(checkbox => {
		    if (checkbox.checked) {
		        ...
		        const tagHTML = `<span class="search-filter__tag" data-name="${groupName}" data-filter="${labelText}" data-value="${checkbox.value}">${groupLabel} > ${labelText}<button type="button" class="search-filter__tag-remove">×</button></span>`;
	...
	if (selectedFiltersContainer) {
		selectedFiltersContainer.addEventListener('click', (e) => {
			if (e.target.classList.contains('search-filter__tag-remove')) {
				...
				// 연결된 체크박스 찾아서 해제
				const checkboxToUncheck = Array.from(allCheckboxes).find(
